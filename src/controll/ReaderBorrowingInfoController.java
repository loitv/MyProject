...
public class ReaderBorrowingInfoController {
	...
	public ReaderBorrowingInfoController(String readerID) {
		...
		// handle events when changing selected item from combo box
		...
		readerbiv.setexportPatternActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					...
					PdfWriter.getInstance(document, new FileOutputStream("D:\\pattern.pdf"));
					document.open();
					...
					document.add(new Paragraph(
							"-------------------------------------------------------------------------------------------------"));
					document.add(new Paragraph("BOOK BORROWING FORM ",
					...
					document.add(new Paragraph(
							"                                                                                     "));
					document.add(new Paragraph("Reader ID: " + readerbiv.getLbReaderID2().getText() + "            "
							+ "Reader Name: " + readerbiv.getLbReaderName1().getText()));
					document.add(new Paragraph(
							"                                                                                     "));
					document.add(new Paragraph("Borrowing Date: " + readerbiv.getTfBorrowDate().getText()
							+ "            Returning Date: " + readerbiv.getTfReturnDate().getText()
							+ "           Deposit: " + readerbiv.getTfDeposit().getText()));
					document.add(new Paragraph(
							"                                                                                     "));
					...
					document.close();
					JOptionPane.showMessageDialog(null, "Borrowing Pattern has been exported!");
				} catch (Exception ex) {
					ex.printStackTrace();
				}
			}
		...
		// handle events for button Open File
		readerbiv.setBtnOpenFileActionListener(new ActionListener() {

			@Override
			public void actionPerformed(ActionEvent arg0) {
				// TODO Auto-generated method stub
				try {
					Desktop.getDesktop().open(new File("D:\\pattern.pdf"));
